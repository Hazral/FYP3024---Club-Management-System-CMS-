...
// Handle CSV export
if (isset($_GET['export']) && $_GET['export'] === 'csv' && isset($_GET['event'])) {
    $eventID = $_GET['event'];
    
    // Verify lecturer owns this event
    $verify_query = "SELECT c.clubID FROM casevents e 
                    INNER JOIN clubsocieties c ON e.clubID = c.clubID 
                    WHERE e.eventID = ? AND c.lectID = ?";
    $verify_stmt = $conn->prepare($verify_query);
    $verify_stmt->execute([$eventID, $lectID]);
    
    if ($verify_stmt->fetch()) {
        // Get event details
        $event_query = "SELECT e.*, c.clubName 
                       FROM casevents e
                       INNER JOIN clubsocieties c ON e.clubID = c.clubID
                       WHERE e.eventID = ?";
        ...
        if ($event['evType'] === 'Private') {
            $participants_query = "SELECT s.studName, s.studNoID, s.studEmail,
                                  COALESCE(a.status, 'Absent') as status,
                                  a.attendDate, a.attendTime, a.remarks,
                                  'Yes' as is_member
                                  FROM membership cm
                                  INNER JOIN student s ON cm.studID = s.studID
                                  LEFT JOIN events_participant ep ON s.studID = ep.studID AND ep.eventID = ?
                                  LEFT JOIN events_attendance a ON ep.partID = a.partID
                                  WHERE cm.clubID = ?
                                  ORDER BY s.studName";
            $stmt = $conn->prepare($participants_query);
            $stmt->execute([$eventID, $event['clubID']]);
        } else {
            $participants_query = "SELECT s.studName, s.studNoID, s.studEmail,
                                  a.status,
                                  a.attendDate, a.attendTime, a.remarks,
                                  CASE WHEN cm.cmID IS NOT NULL THEN 'Yes' ELSE 'No' END as is_member
                                  FROM events_participant ep
                                  INNER JOIN student s ON ep.studID = s.studID
                                  INNER JOIN events_attendance a ON ep.partID = a.partID
                                  LEFT JOIN membership cm ON s.studID = cm.studID AND cm.clubID = ?
                                  WHERE ep.eventID = ?
                                  ORDER BY s.studName";
            $stmt = $conn->prepare($participants_query);
            $stmt->execute([$event['clubID'], $eventID]);
        }
        $participants = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Generate CSV
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="event_attendance_' . $eventID . '_' . date('Ymd_His') . '.csv"');
        ...
        fputcsv($output, ['Event Attendance Report']);
        fputcsv($output, ['Generated On', date('F d, Y g:i A')]);
        fputcsv($output, []);
        fputcsv($output, ['Event Title', $event['evTitle']]);
        fputcsv($output, ['Club/Society', $event['clubName']]);
        fputcsv($output, ['Event Type', ucfirst($event['evType'])]);
        fputcsv($output, ['Date', date('F d, Y', strtotime($event['evDate']))]);
        fputcsv($output, ['Time', date('g:i A', strtotime($event['evTime']))]);
        fputcsv($output, ['Location', $event['evLocation']]);
        fputcsv($output, ['Description', $event['evDescription']]);
        fputcsv($output, []);
        ...
        fputcsv($output, ['Total Participants', $total_count]);
        ...
        // Participants table
        fputcsv($output, ['Student Name', 'Student ID', 'Email', 'Status', 'Check-in Date', 'Check-in Time', 'Club Member', 'Remarks']);
        foreach ($participants as $participant) {
            fputcsv($output, [
                ...
                $participant['status'],
                $participant['attendDate'] ? date('M d, Y', strtotime($participant['attendDate'])) : '-',
                $participant['attendTime'] ? date('g:i A', strtotime($participant['attendTime'])) : '-',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        try {
            ...
            switch ($_POST['action']) {
                case 'mark_attendance':
                    // Verify the event belongs to lecturer's club
                    $verify_query = "SELECT c.clubID, e.evType FROM casevents e 
                                    INNER JOIN clubsocieties c ON e.clubID = c.clubID 
                                    WHERE e.eventID = ? AND c.lectID = ?";
                    $verify_stmt = $conn->prepare($verify_query);
                    $verify_stmt->execute([$_POST['eventID'], $lectID]);
                    $event_info = $verify_stmt->fetch(PDO::FETCH_ASSOC);
                    
                    if (!$event_info) {
                        throw new Exception('You do not have permission to mark attendance for this event.');
                    }

                    // Check if participant record exists
                    $check_query = "SELECT partID FROM events_participant WHERE eventID = ? AND studID = ?";
                    $check_stmt = $conn->prepare($check_query);
                    $check_stmt->execute([$_POST['eventID'], $_POST['studID']]);
                    $existing_part = $check_stmt->fetch(PDO::FETCH_ASSOC);
                    
                    if ($existing_part) {
                        // Update existing attendance
                        $check_attend = "SELECT attendanceID FROM events_attendance WHERE partID = ?";
                        $check_attend_stmt = $conn->prepare($check_attend);
                        $check_attend_stmt->execute([$existing_part['partID']]);
                        
                        if ($check_attend_stmt->fetch()) {
                            throw new Exception('Attendance already marked for this student.');
                        }
                        
                        $partID = $existing_part['partID'];
                    } else {
                        // Create participant record
                        $insert_part = "INSERT INTO events_participant (eventID, studID, partDate) VALUES (?, ?, NOW())";
                        $part_stmt = $conn->prepare($insert_part);
                        $part_stmt->execute([$_POST['eventID'], $_POST['studID']]);
                        $partID = $conn->lastInsertId();
                    }

                    // Create attendance record
                    $remarks = isset($_POST['remarks']) ? trim($_POST['remarks']) : null;
                    $insert_attend = "INSERT INTO events_attendance (partID, status, attendTime, attendDate, remarks) 
                                     VALUES (?, 'Present', CURRENT_TIME(), CURRENT_DATE(), ?)";
                    $attend_stmt = $conn->prepare($insert_attend);
                    $attend_stmt->execute([$partID, $remarks]);
                ...
                case 'mark_absent':
                    ...
                    if (!$verify_stmt->fetch()) {
                        throw new Exception('You do not have permission to modify attendance for this event.');
                    }
                    ...
                    $check_attend = "SELECT attendanceID FROM events_attendance WHERE partID = ?";
                    $check_attend_stmt = $conn->prepare($check_attend);
                    $check_attend_stmt->execute([$partID]);
                    ...
                    $remarks = isset($_POST['remarks']) ? trim($_POST['remarks']) : null;
                    $insert_attend = "INSERT INTO events_attendance (partID, status, attendDate, remarks) 
                ...
                case 'update_attendance':
                    ...
                    $verify_query = "SELECT c.clubID FROM casevents e 
                                    INNER JOIN clubsocieties c ON e.clubID = c.clubID 
                                    INNER JOIN events_participant ep ON e.eventID = ep.eventID
                                    WHERE ep.partID = ? AND c.lectID = ?";
                    $verify_stmt = $conn->prepare($verify_query);
                    $verify_stmt->execute([$_POST['partID'], $lectID]);
                    ...
                    if ($new_status === 'Present') {
                        $update = "UPDATE events_attendance SET status = ?, attendTime = CURRENT_TIME(), 
                                  attendDate = CURRENT_DATE(), remarks = ? WHERE partID = ?";
                    } else {
                        $update = "UPDATE events_attendance SET status = ?, attendTime = NULL, 
                                  attendDate = CURRENT_DATE(), remarks = ? WHERE partID = ?";
                    }
                    $stmt = $conn->prepare($update);
                    $stmt->execute([$new_status, $remarks, $_POST['partID']]);
                ...
                case 'delete_attendance':
                    ...
                    // Delete attendance record (participant record remains)
                    $delete = "DELETE FROM events_attendance WHERE partID = ?";
                    $stmt = $conn->prepare($delete);
                    $stmt->execute([$_POST['partID']]);
        ...
        if (isset($_POST['eventID'])) {
            $redirect_params[] = 'event=' . $_POST['eventID'];
        }
        if (isset($_GET['month'])) {
        ...
        $redirect_url = "event_log.php" . (!empty($redirect_params) ? "?" . implode('&', $redirect_params) : "");
...
// Get selected event for attendance
$selected_event = isset($_GET['event']) && $_GET['event'] !== '' ? $_GET['event'] : null;

// Fetch events for the selected month from lecturer's clubs
$events_query = "SELECT e.*, c.clubName, c.clubID,
                 COUNT(DISTINCT CASE WHEN a.status = 'Present' THEN a.attendanceID END) as present_count,
                 COUNT(DISTINCT CASE WHEN a.status = 'Absent' THEN a.attendanceID END) as absent_count,
                 COUNT(DISTINCT ep.partID) as total_registered
                 FROM casevents e
                 INNER JOIN clubsocieties c ON e.clubID = c.clubID
...
if ($club_filter !== null) {
    $events_query .= " AND e.clubID = ?";
    $params[] = $club_filter;
}

$events_query .= " GROUP BY e.eventID ORDER BY e.evDate, e.evTime";
...
// If an event is selected, get its details and participants
$event_details = null;
$event_members = [];
if ($selected_event) {
    $event_query = "SELECT e.*, c.clubName, c.clubID
                    FROM casevents e
                    INNER JOIN clubsocieties c ON e.clubID = c.clubID
                    WHERE e.eventID = ? AND c.lectID = ?";
    $stmt = $conn->prepare($event_query);
    $stmt->execute([$selected_event, $lectID]);
    $event_details = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($event_details) {
        // Get participants based on event type
        if ($event_details['evType'] === 'Private') {
            // Private event: show all club members with attendance status
            $members_query = "SELECT s.studID, s.studName, s.studNoID, s.studEmail,
                            ep.partID, a.attendanceID, a.status, a.attendDate, a.attendTime, a.remarks,
                            1 as is_member
                            FROM membership cm
                            INNER JOIN student s ON cm.studID = s.studID
                            LEFT JOIN events_participant ep ON s.studID = ep.studID AND ep.eventID = ?
            ...
            $stmt->execute([$selected_event, $event_details['clubID']]);
        } else {
            // Public event: show students who registered/attended
            $members_query = "SELECT s.studID, s.studName, s.studNoID, s.studEmail,
                            ep.partID, a.attendanceID, a.status, a.attendDate, a.attendTime, a.remarks,
                            CASE WHEN cm.cmID IS NOT NULL THEN 1 ELSE 0 END as is_member
                            FROM events_participant ep
                            INNER JOIN student s ON ep.studID = s.studID
                            LEFT JOIN events_attendance a ON ep.partID = a.partID
                            LEFT JOIN membership cm ON s.studID = cm.studID AND cm.clubID = ?
                            ...
                            ORDER BY s.studName";
            $stmt = $conn->prepare($members_query);
            $stmt->execute([$event_details['clubID'], $selected_event]);
        }
        $event_members = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
}
...
<head>
    ...
    <title>Event Attendance Management</title>
...
<body>
                ...
                <?php if (empty($clubs)): ?>
                    <div class="card bg-base-100 shadow-xl fade-in">
                        <div class="card-body text-center py-12">
                            ...
                            <p class="text-gray-400 mt-2">You need to be in charge of a club to manage event attendance.</p>
                        </div>
                    </div>
                <?php else: ?>
                    <!-- Statistics Summary -->
                    <?php if (!$selected_event): ?>
                        <?php
                        $total_present = array_sum(array_column($events, 'present_count'));
                        $total_absent = array_sum(array_column($events, 'absent_count'));
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                            <div class="stats shadow stats-card fade-in">
                                <div class="stat">
                                    ...
                                    <div class="stat-value text-primary"><?= count($events) ?></div>
                                ...
                                for ($day = 1; $day <= $days_in_month; $day++) {
                                    ...
                                    if ($has_events) {
                                        foreach ($events_by_date[$date] as $event) {
                                            $event_datetime = strtotime($event['evDate'] . ' ' . $event['evTime']);
                                            $current_datetime = time();
                                            $can_mark_attendance = $event_datetime <= $current_datetime;
                                            
                                            if ($event['evType'] === 'Private') {
                                                $badge_color = $can_mark_attendance ? 'badge-success' : 'badge-info';
                                                $icon = '🔒';
                                            } else {
                                                $badge_color = $can_mark_attendance ? 'badge-warning' : 'badge-accent';
                                                $icon = '🌐';
                                            }
                                            
                                            $attendance_info = '';
                                            if ($event['present_count'] > 0 || $event['absent_count'] > 0) {
                                                $attendance_info = ' (' . $event['present_count'] . '/' . ($event['present_count'] + $event['absent_count']) . ')';
                                            }
                                            
                                            echo '<a href="?month=' . $selected_month . '&year=' . $selected_year . 
                                                 ($club_filter ? '&club=' . $club_filter : '') . 
                                                 '&event=' . $event['eventID'] . '" class="badge ' . $badge_color . ' event-badge gap-1" title="' . 
                                                 htmlspecialchars($event['evTitle']) . ' - ' . 
                                                 htmlspecialchars($event['clubName']) . $attendance_info . '">' .
                                                 $icon . ' ' .
                                                 htmlspecialchars(substr($event['evTitle'], 0, 12)) . 
                                                 (strlen($event['evTitle']) > 12 ? '...' : '') .
                                                 '</a>';
                                        }
                                    }
                    ...
                    <?php if ($event_details): ?>
                        <?php
                        $event_datetime = strtotime($event_details['evDate'] . ' ' . $event_details['evTime']);
                        $current_datetime = time();
                        $can_mark_attendance = $event_datetime <= $current_datetime && $event_details['evType'] === 'Private';
                        $present_count = count(array_filter($event_members, function($m) { return $m['status'] === 'Present'; }));
                        $absent_count = count(array_filter($event_members, function($m) { return $m['status'] === 'Absent'; }));
                        $not_marked = count(array_filter($event_members, function($m) { return $m['attendanceID'] === null; }));
                            ...
                            <div class="card-body">
                                ...
                                <div class="flex flex-wrap justify-between items-start gap-4 mb-6">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-3">
                                            <h2 class="card-title text-3xl"><?= htmlspecialchars($event_details['evTitle']) ?></h2>
                                            <?php if ($event_details['evType'] === 'Private'): ?>
                                                <span class="text-2xl" title="Private Event">🔒</span>
                                            <?php else: ?>
                                                <span class="text-2xl" title="Public Event">🌐</span>
                                            <?php endif; ?>
                                        </div>
                                        <div class="flex flex-wrap gap-2">
                                            <span class="badge badge-primary badge-lg gap-2">
                                                ...
                                                <?= htmlspecialchars($event_details['clubName']) ?>
                                            </span>
                                            <span class="badge badge-lg <?= $event_details['evType'] === 'Private' ? 'badge-success' : 'badge-warning' ?> gap-2">
                                                <?= ucfirst($event_details['evType']) ?> Event
                                            </span>
                                            ...
                                            <?php elseif ($event_details['evType'] === 'Private'): ?>
                                                <span class="badge badge-info badge-lg">Upcoming Event</span>
                                            <?php else: ?>
                                                <span class="badge badge-accent badge-lg">Public Event</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    
                                    <div class="flex gap-2">
                                        <a href="?export=csv&event=<?= $event_details['eventID'] ?>" 
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
                                    <div class="stats shadow">
                                        <div class="stat">
                                            ...
                                            <div class="stat-value text-lg"><?= date('M d, Y', strtotime($event_details['evDate'])) ?></div>
                                            <div class="stat-desc"><?= date('g:i A', strtotime($event_details['evTime'])) ?></div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="stats shadow">
                                        <div class="stat">
                                            ...
                                            <div class="stat-desc"><?= $event_details['evType'] === 'Private' ? 'Club members' : 'Registered' ?></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="alert alert-info">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm"><?= nl2br(htmlspecialchars($event_details['evDescription'])) ?></div>
                                    </div>
                                </div>

                                <div class="divider">Participant Attendance Management</div>

                                <?php if ($event_details['evType'] === 'Public'): ?>
                                    <div class="alert alert-warning mb-4">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                        </svg>
                                        <span><strong>Public Event:</strong> This shows students who registered. You can update their attendance status and add remarks.</span>
                                    </div>
                                <?php elseif (!$can_mark_attendance): ?>
                                    <div class="alert alert-info mb-4">
                                        ...
                                        <span>Attendance can only be marked during or after the event time.</span>
                                    </div>
                                <?php endif; ?>

                                <!-- Attendance Table -->
                                <div class="overflow-x-auto">
                                    <?php if (empty($event_members)): ?>
                                        ...
                                        <table class="table table-zebra">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Check-in Time</th>
                                                    <th>Remarks</th>
                                                    <?php if ($can_mark_attendance || $event_details['evType'] === 'Public'): ?>
                                                        <th>Actions</th>
                                                    <?php endif; ?>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php $counter = 1; ?>
                                                <?php foreach ($event_members as $member): ?>
                                                    <tr class="hover">
                                                        <td><?= $counter++ ?></td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            ...
                                                            <?php else: ?>
                                                                <span class="badge badge-ghost">Non-Member</span>
                                                            <?php endif; ?>
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td class="text-sm">
                                                            <?php if ($member['attendDate'] && $member['attendTime']): ?>
                                                                <div class="flex flex-col">
                                                                    <span><?= date('M d, Y', strtotime($member['attendDate'])) ?></span>
                                                                    <span class="text-xs text-gray-500"><?= date('g:i A', strtotime($member['attendTime'])) ?></span>
                                                                </div>
                                                            <?php elseif ($member['attendDate']): ?>
                                                                <?= date('M d, Y', strtotime($member['attendDate'])) ?>
                                                            <?php else: ?>
                                                        ...
                                                        <?php if ($can_mark_attendance || $event_details['evType'] === 'Public'): ?>
                                                            <td>
                                                                <div class="flex gap-1">
                                                                    <?php if ($member['attendanceID']): ?>
                                                                        <!-- Edit Button -->
                                                                        <button onclick="openEditModal(<?= htmlspecialchars(json_encode($member)) ?>, <?= $event_details['eventID'] ?>)" 
                                                                        ...
                                                                        <form method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this attendance record?');">
                                                                            ...
                                                                            <input type="hidden" name="partID" value="<?= $member['partID'] ?>">
                                                                            <input type="hidden" name="eventID" value="<?= $event_details['eventID'] ?>">
                                                                    ...
                                                                    <?php else: ?>
                                                                        <!-- Mark Present Button -->
                                                                        <button onclick="openMarkModal(<?= $member['studID'] ?>, '<?= htmlspecialchars($member['studName']) ?>', 'Present', <?= $event_details['eventID'] ?>)" 
                                                                        ...
                                                                        <!-- Mark Absent Button -->
                                                                        <button onclick="openMarkModal(<?= $member['studID'] ?>, '<?= htmlspecialchars($member['studName']) ?>', 'Absent', <?= $event_details['eventID'] ?>)" 
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="flex items-center gap-3 p-3 bg-base-200 rounded-lg">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500">Only club members, attendance can be marked manually</div>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3 p-3 bg-base-200 rounded-lg">
                                    <span class="badge badge-warning">🌐</span>
                                    <div>
                                        <div class="font-bold">Public Event</div>
                                        <div class="text-xs text-gray-500">Open to all, shows registered participants with membership status</div>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3 p-3 bg-base-200 rounded-lg">
    ...
    <dialog id="markModal" class="modal">
        <div class="modal-box">
            ...
            <form method="POST" id="markForm">
                ...
                <input type="hidden" name="eventID" id="markEventID" value="">
    ...
    <dialog id="editModal" class="modal">
        <div class="modal-box">
            ...
            <form method="POST" id="editForm">
                ...
                <input type="hidden" name="partID" id="editPartID" value="">
    ...
    <script>
        ...
        function openMarkModal(studID, studName, status, eventID) {
            document.getElementById('markStudID').value = studID;
            document.getElementById('markEventID').value = eventID;
        ...
        function openEditModal(member, eventID) {
            document.getElementById('editPartID').value = member.partID;
