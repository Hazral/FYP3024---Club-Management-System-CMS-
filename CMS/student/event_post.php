...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'participate') {
        try {
            ...
            $query = "SELECT * FROM events_participant WHERE eventID = ? AND studID = ?";
            ...
            if ($stmt->fetch()) {
                ...
                $query = "SELECT e.evCapacity, COUNT(ep.partID) as current_count 
                          FROM casevents e 
                          LEFT JOIN events_participant ep ON e.eventID = ep.eventID 
                          WHERE e.eventID = ? 
                          GROUP BY e.eventID, e.evCapacity";
                $stmt = $conn->prepare($query);
                $stmt->execute([$event_id]);
                $capacity_check = $stmt->fetch(PDO::FETCH_ASSOC);
                
                if ($capacity_check && $capacity_check['current_count'] >= $capacity_check['evCapacity']) {
                    $_SESSION['error'] = "Sorry, this event is already full!";
                } else {
                    // Register for event
                    $query = "INSERT INTO events_participant (eventID, studID, partDate) VALUES (?, ?, NOW())";
                    $stmt = $conn->prepare($query);
                    $stmt->execute([$event_id, $student_id]);
        ...
        header("Location: event_post.php?id=" . $event_id);
        exit();
    }
    
    if ($_POST['action'] === 'cancel_participation') {
        try {
            $query = "DELETE FROM events_participant WHERE eventID = ? AND studID = ?";
            $stmt = $conn->prepare($query);
            $stmt->execute([$event_id, $student_id]);
...
// Get event details
try {
    $query = "SELECT e.eventID, e.evTitle, e.evDescription, e.evLocation, e.evPosted_at, e.evType, 
              e.evImg, e.evVid, e.evTime, e.evDate, e.evCapacity,
              c.clubName, c.clubID, c.clubLogo, c.clubDescription, l.lectName,
              (SELECT COUNT(*) FROM events_participant WHERE eventID = e.eventID) as participant_count
              FROM casevents e 
...
try {
    ...
    $stmt = $conn->prepare($query);
    $stmt->execute([$student_id, $event['clubID']]);
...
try {
    ...
    $stmt = $conn->prepare($query);
    $stmt->execute([$event_id, $student_id]);
    $is_participating = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $is_participating = false;
}

// Check if event is full
$is_full = $event['participant_count'] >= $event['evCapacity'];
...
// Get related events from same club
try {
    $query = "SELECT eventID, evTitle, evDate, evTime, evImg, evLocation
              FROM casevents 
              WHERE clubID = ? AND eventID != ? AND evDate >= CURDATE()
              ORDER BY evDate ASC 
              LIMIT 3";
    $stmt = $conn->prepare($query);
    $stmt->execute([$event['clubID'], $event_id]);
...
try {
    ...
    $current_student = ['studName' => 'Student', 'studProfileImg' => ''];
}
...
try {
    $query = "SELECT s.studName, s.studProfileImg, ep.partDate
              FROM events_participant ep
              JOIN student s ON ep.studID = s.studID
              WHERE ep.eventID = ?
...
<head>
    ...
    <title><?php echo htmlspecialchars($event['evTitle']); ?> - Student Club Management</title>
...
<body class="bg-base-200">
    <div class="drawer">
        ...
        <div class="drawer-content flex flex-col">
            ...
            <div class="toast toast-top toast-end z-50 no-print">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="card bg-base-100 shadow-xl animate-fade-in">
                            ...
                            <figure class="relative">
                                    ...
                                    <?php foreach ($media_items as $index => $media): ?>
                                        <div id="slide<?php echo $index + 1; ?>" class="carousel-item relative w-full">
                                            ...
                                            <?php if (count($media_items) > 1): ?>
                                                <div class="absolute flex justify-between transform -translate-y-1/2 left-5 right-5 top-1/2 no-print">
                                ...
                                <?php if (count($media_items) > 1): ?>
                                    <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2 z-10 no-print">
                                    ...
                                    <div class="absolute top-4 right-4 z-10 no-print">
                                ...
                                <div class="absolute top-4 left-4 z-2 flex gap-2 no-print">
                                    <?php 
                                    $hasImage = false;
                                    $hasVideo = false;
                            ...
                            <div class="card-body">
                                ...
                                <div class="flex items-center gap-2 mb-4 flex-wrap">
                                    <span class="badge bg-gradient-to-r from-green-500 to-emerald-600 text-white border-none">
                                    ...
                                    <?php endif; ?>
                                    <?php if ($is_past): ?>
                                    <span class="badge badge-ghost">Past Event</span>
                                    <?php elseif ($is_full): ?>
                                    <span class="badge badge-error">Full</span>
                                    <?php endif; ?>
                                    <span class="text-sm text-gray-500 ml-auto">
                                        Posted on <?php echo date('F j, Y', strtotime($event['evPosted_at'])); ?>
                                    </span>
                                </div>
                                ...
                                <!-- Event Details Grid -->
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 p-4 bg-base-200 rounded-lg">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="font-bold"><?php echo date('l, F j, Y', strtotime($event['evDate'])); ?></p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="font-bold"><?php echo date('g:i A', strtotime($event['evTime'])); ?></p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="font-bold"><?php echo htmlspecialchars($event['evLocation']); ?></p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="font-bold">
                                                <span class="<?php echo $is_full ? 'text-error' : 'text-success'; ?>">
                                                    <?php echo $event['participant_count']; ?>
                                                </span> / <?php echo $event['evCapacity']; ?>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-3 mb-6 p-4 bg-base-200 rounded-lg">
                                    ...
                                    <div>
                                        <h3 class="font-bold">
                                            <a href="club_profile.php?id=<?php echo $event['clubID']; ?>" class="link link-hover text-purple-600">
                                                <?php echo htmlspecialchars($event['clubName']); ?>
                                            </a>
                                        </h3>
                                        <?php if (!empty($event['lectName'])): ?>
                                            <p class="text-sm text-gray-600">Advisor: <?php echo htmlspecialchars($event['lectName']); ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                ...
                                <div class="content-section prose max-w-none">
                                    <h3 class="text-xl font-bold mb-3">Event Description</h3>
                                    <p><?php echo nl2br(htmlspecialchars($event['evDescription'])); ?></p>
                                </div>

                                <div class="divider"></div>

                                <!-- Actions -->
                                <div class="card-actions justify-between items-center flex-wrap gap-4">
                                    <?php if ($is_past): ?>
                                    ...
                                    <?php elseif ($is_participating): ?>
                                        <div class="flex items-center gap-3 flex-wrap">
                                            ...
                                            <form method="POST" onsubmit="return confirm('Are you sure you want to cancel your registration?');" class="no-print">
                                    ...
                                    <?php elseif ($is_full): ?>
                                        <div class="alert alert-error">
                                            ...
                                            <span>This event is full - No more registrations available</span>
                                        </div>
                                    <?php else: ?>
                                        <form method="POST" class="no-print">
                                            <input type="hidden" name="action" value="participate">
                                            <button type="submit" class="btn bg-gradient-to-r from-green-600 to-emerald-600 text-white border-none hover:shadow-lg">
                        ...
                        <!-- Comments Section -->
                        <div class="card bg-base-100 shadow-xl animate-fade-in no-print">
                            <div class="card-body">
                                            ...
                                            <div class="flex gap-3">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <div class="flex items-center gap-4 text-sm">
                                                        ...
                                                        <button onclick="toggleReplyForm(<?php echo $comment['commentID']; ?>)" class="text-gray-500 hover:text-green-600 transition">
                        ...
                        <div class="card bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-xl animate-fade-in">
                            <div class="card-body">
                                <div class="flex items-center gap-3 mb-4">
                                    <div class="avatar">
                                        <div class="w-16 h-16 rounded-full ring ring-white">
                                            <?php if (!empty($event['clubLogo'])): ?>
                                            <img src="../<?php echo htmlspecialchars($event['clubLogo']); ?>" 
                                                alt="<?php echo htmlspecialchars($event['clubName']); ?>">
                                            <?php else: ?>
                                            <div class="bg-white text-purple-600 flex items-center justify-center text-2xl font-bold">
                                                <?php echo strtoupper(substr($event['clubName'], 0, 2)); ?>
                                            </div>
                                            <?php endif; ?>
                                        </div>
                                    ...
                                    <div>
                                        <h3 class="font-bold text-lg"><?php echo htmlspecialchars($event['clubName']); ?></h3>
                                        <?php if (!empty($event['lectName'])): ?>
                                        <p class="text-sm opacity-90">Advisor: <?php echo htmlspecialchars($event['lectName']); ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                
                                <?php if (!empty($event['clubDescription'])): ?>
                                <p class="text-sm opacity-90 mb-4"><?php echo htmlspecialchars(substr($event['clubDescription'], 0, 150)); ?><?php echo strlen($event['clubDescription']) > 150 ? '...' : ''; ?></p>
                                <?php endif; ?>
                                
                                <a href="club_profile.php?id=<?php echo $event['clubID']; ?>" class="btn btn-sm bg-white text-purple-600 border-none hover:bg-gray-100">
                                    View Club Profile
    ...
    <script>
                    ...
                    <div class="flex gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-4 text-sm">
                                ...
                                <button onclick="toggleReplyForm(${comment.id})" class="text-gray-500 hover:text-green-600 transition">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 inline mr-1" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M7.707 3.293a1 1 0 010 1.414L5.414 7H11a7 7 0 017 7v2a1 1 0 11-2 0v-2a5 5 0 00-5-5H5.414l2.293 2.293a1 1 0 11-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                    </svg>
                                    Reply
        ...
        function shareEvent() {
            ...
            const title = '<?php echo addslashes($event['evTitle'] ?? ''); ?>';
